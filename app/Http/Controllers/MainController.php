...
class MainController extends Controller {
	...
	/**
	 * Show the Add Gallery view.
	 *
	 * @return Response
	 */
	public function getAddGallery(Request $request)
    {
		$user = null;
		$nope = false;
		$v = "";
	...
	public function getGallery(Request $request)
    {
		...
		if(Auth::check())
		{
			...
			if($this->helpers->isAdmin($user))
			{
				...
				$v = "gallery";
				$req = $request->all();
                $gallery = $this->helpers->getGallery();
				#dd($reviews);
                array_push($cpt,'gallery');
                }
				else
				{
					session()->flash("permissions-status-error","ok");
	 ...
	 * @return Response
	 */
	public function getRemoveGallery(Request $request)
    {
		$user = null;
		if(Auth::check())
		{
	...
	/**
	 * Show the Add Shipping view.
	 *
	 * @return Response
	 */
	public function getAddShipping(Request $request)
    {
		$user = null;
		$nope = false;
		$v = "";
	...
	public function postAddShipping(Request $request)
    {
		...
		if(Auth::check())
		{
			...
			if($this->helpers->isAdmin($user))
			{
				...
				{
					$ret = $this->helpers->createShipping($req);
					$ss = "add-shipping-status";
					if($ret == "error") $ss .= "-error";
					session()->flash($ss,"ok");
	...
	public function postEditShipping(Request $request)
    {
		...
		if(Auth::check())
		{
			...
			if($this->helpers->isAdmin($user))
			{
				...
				{
					$ret = $this->helpers->updateShipping($req);
					$ss = "update-shipping-status";
					if($ret == "error") $ss .= "-error";
					session()->flash($ss,"ok");
			        return redirect()->intended("shipping");
				}
	...
	/**
	 * Handle remove Shipping.
	 *
	 * @return Response
	 */
	public function getRemoveShipping(Request $request)
    {
		$user = null;
		if(Auth::check())
		{
			...
			if($this->helpers->isAdmin($user))
			{
				...
				if($hasPermission)
				{
				    ...
				    {   
					  $ret = $this->helpers->removeShipping($req['s']);
					  $ss = "remove-shipping-status";
					  if($ret == "error") $ss .= "-error";
					  session()->flash($ss,"ok");
			          return redirect()->intended("shipping");
				    }
